...
malicious sofware. This software may run software like in such
a way that the user is prompted to sign a valid transaction, but secretly act maliciously.
Specifically, the computer can do the following:

1. Lie to the user about which actions it is performing. *Example: the user
   wants to read the public key at index #1, but the computer secretly reads the public key at index #2.*
2. Lie to the user about what it received from the Nano S. *Example: the Nano
   S generates public key X, but the computer displays public key Y.*
3. Exfiltrate data supplied by the user or the Nano S. *Example: the user
   generates public keys A and B; the computer "phones home" to report that A and
   B are owned by the same person.*
4. Refuse to comply with the user's requests. *Example: the user wants to read the public key at index #1, 
but the computer does nothing.*
